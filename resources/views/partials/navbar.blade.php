...
<div id="fullscreen-menu"
    class="fixed inset-0 z-[400] flex flex-col justify-center px-8 md:px-32"
    style="background-color: #F1F1F1; background: #F1F1F1; transform: translateY(-100%); transition: transform 1s cubic-bezier(0.85,0,0.15,1);">

    <button id="close-menu-btn"
        class="absolute top-10 right-8 md:right-16 font-sans text-[10px] tracking-[0.25em] uppercase font-bold transition-colors"
        style="color: #25225E; background: transparent; border: none; cursor: none;"
        onmouseenter="this.style.color='#FFB150'"
        onmouseleave="this.style.color='#25225E'">
        [ Close ]
    </button>
    ...
    <div class="absolute top-10 left-8 md:left-16 flex items-center gap-4">
        <button id="lang-switcher"
            ...
            class="font-sans text-[10px] tracking-[0.25em] uppercase font-bold transition-colors"
            style="color: rgba(37,34,94,0.5); background: transparent; border: none; cursor: none;"
    ...
    <div class="flex flex-col space-y-2 font-serif" style="font-family: 'Instrument Serif', serif;">
        @php
            $allMenuItems = [
                ['title' => 'About',         'url' => '/',             'i18n' => 'menu_about'],
                ['title' => 'Our Works',      'url' => '#',             'i18n' => 'menu_our_works', 'isLabel' => true],
                ['title' => '↳ Films',        'url' => '/films',        'i18n' => 'menu_films',        'sub' => true],
                ['title' => '↳ Web Series',   'url' => '/serial',       'i18n' => 'menu_web_series',   'sub' => true],
                ['title' => '↳ Television',   'url' => '/tv',           'i18n' => 'menu_television',   'sub' => true],
                ['title' => '↳ Documentaries','url' => '/documentary',  'i18n' => 'menu_documentaries','sub' => true],
        ...
        @foreach($allMenuItems as $item)
            @if(isset($item['isLabel']))
                <span class="menu-link font-sans text-[10px] tracking-[0.3em] uppercase font-bold mt-4 mb-1 opacity-0"
                    style="color: rgba(37,34,94,0.4); transform: translateY(50px) skewY(5deg);">
                    <span data-i18n="{{ $item['i18n'] }}">{{ $item['title'] }}</span>
                </span>
            @elseif(isset($item['sub']))
                <a href="{{ $item['url'] }}"
                    class="menu-link inline-block w-max transition-all duration-500 opacity-0"
                    style="font-size: clamp(2rem, 5vw, 4rem); color: #25225E; transform: translateY(50px) skewY(5deg); text-decoration: none; line-height: 1.1;"
            ...
            @else
                <a href="{{ $item['url'] }}"
                    class="menu-link inline-block w-max transition-all duration-500 opacity-0"
                    style="font-size: clamp(3rem, 7vw, 6rem); color: #25225E; transform: translateY(50px) skewY(5deg); text-decoration: none; line-height: 1.05;"
                    onmouseenter="this.style.fontStyle='italic'; this.style.color='#FFB150'"
                    onmouseleave="this.style.fontStyle=''; this.style.color='#25225E'">
                    <span data-i18n="{{ $item['i18n'] }}">{{ $item['title'] }}</span>
                </a>
    ...
    <div class="absolute bottom-10 right-8 md:right-16">
        <span class="font-sans text-[9px] tracking-[0.2em] uppercase" style="color: rgba(37,34,94,0.4);">EST. 2020 • Jakarta, ID</span>
    </div>
</div>
...
<nav id="unified-navbar"
    class="fixed top-0 left-0 w-full z-[300] flex justify-between items-start px-8 md:px-16 py-10 transition-all duration-500 mix-blend-multiply"
    style="color: #25225E;">

    {{-- Brand (Left) --}}
    <a href="{{ route('welcome') }}"
        class="font-serif text-3xl tracking-tight leading-none relative z-10 transition-all"
        style="font-family: 'Instrument Serif', serif; color: #25225E; text-decoration: none; cursor: none;"
        onmouseenter="this.style.color='#FFB150'"
        onmouseleave="this.style.color='#25225E'">
    ...
    {{-- Right Controls --}}
    <div class="flex gap-12 md:gap-16 font-sans text-[10px] tracking-[0.25em] uppercase font-bold items-start" style="color: rgba(37,34,94,0.6);">
        <div class="hidden md:flex flex-col gap-1 text-right">
            <span>Est. 2020</span>
            <span>Jakarta, ID</span>
        </div>
        <button id="menu-open-btn"
            class="hamburger-btn hover:text-brand-orange transition-colors relative z-10"
            style="background: transparent; border: none; cursor: none; color: rgba(37,34,94,0.6);"
            onmouseenter="this.style.color='#FFB150'"
            onmouseleave="this.style.color='rgba(37,34,94,0.6)'">
            [ Menu ]
        </button>
    </div>
...
(function () {
    ...
    window.__langSwitcherHover = function(btn, isEnter) {
        ...
        if (isEnter) {
            ...
            btn.style.color = 'rgba(37,34,94,0.9)';
        } else {
            label.textContent = currentLang.toUpperCase();
            btn.style.color = 'rgba(37,34,94,0.5)';
        }
    };
