...
<div class="relative z-20 px-8 md:px-16 pb-12 max-w-[1400px] mx-auto w-full">
    <div class="flex flex-col items-start gap-2 mt-12 md:mt-16">
        @php
            ...
            if (
                !empty($films->link) &&
                preg_match('/(?:youtube\.com\/(?:[^\/]+\/.+\/|(?:v|e(?:mbed)?)\/|.*[?&]v=)|youtu\.be\/)([^"&?\/\s]{11})/i', $films->link, $match)
            ) {
                $video_id = $match[1];
            }
        @endphp
        ...
        @if($video_id)
            <button
                ...
                class="inline-flex items-center px-4 py-2 -mt-3 rounded-md border border-[#F36B21] bg-[#F36B21] text-white shadow-sm font-sans text-xs font-bold tracking-widest uppercase transition-all duration-300 hover:bg-white hover:border-[#F36B21] hover:text-[#F36B21] cursor-none hover-target hero-reveal">
                Tonton Trailer
            </button>
        @endif
            ...
            <div class="max-w-[1400px] mx-auto flex flex-col lg:flex-row gap-16 lg:gap-24">
                ...
                <div class="w-full lg:w-2/5">
                    ...
                    <div class="flex flex-col gap-4">
                        @php
                            ...
                            if($recs->count() < 3) {
                                ...
                                $recs = $recs->concat(collect([
                                    (object)['title' => 'PATAH HATI YANG KUPILIH', 'release_date' => '2025-01-01', 'cast' => 'Prilly Latuconsina, Bryan Domani', 'slug' => '#', 'cover' => ''],
                                    (object)['title' => 'PATAH HATI YANG KUPILIH', 'release_date' => '2025-01-01', 'cast' => 'Prilly Latuconsina, Bryan Domani', 'slug' => '#', 'cover' => ''],
                                    (object)['title' => 'PATAH HATI YANG KUPILIH', 'release_date' => '2025-01-01', 'cast' => 'Prilly Latuconsina, Bryan Domani', 'slug' => '#', 'cover' => ''],
                                ]))->take(3);
                            }
                        ...
                        <a href="{{ $item->slug == '#' ? '#' : route('detail-film', $item->slug) }}" class="flex items-center gap-5 p-5 rounded-2xl border border-[#131b4d]/10 hover:border-[#F36B21] transition-colors cursor-none hover-target group reveal-rec bg-white">
