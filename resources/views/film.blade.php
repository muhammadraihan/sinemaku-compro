...
@section('title', 'Film | Sinemaku Pictures')
...
@section('content')
    ...
    <div id="editorial-wrapper" class="text-brand-deepbreath relative w-full font-sans">
                    ...
                    <div id="film-slider" class="hero-title-reel">
                        @foreach($heroFilms as $i => $item)
                            <div class="film-nav-item hero-title-item {{ $i === 0 ? 'is-active' : '' }}"
                                data-index="{{ $i }}">
                                <a href="{{ route('detail-film',$item->slug) }}" class="hero-title-link">
                                    <h2 class="film-title font-peckham text-3xl md:text-5xl uppercase {{ $i === 0 ? 'text-white' : 'text-white/40' }}">
                                        @i18n($item,'title')
                                    </h2>
                        ...
                        @if($heroFilms->count() > 1)
                            ...
                            <div class="hero-title-item hero-title-item-clone" aria-hidden="true">
                                <a href="{{ route('detail-film',$firstHeroFilm->slug) }}" class="hero-title-link" tabindex="-1">
                                    <h2 class="film-title font-peckham text-3xl md:text-5xl uppercase text-white/40">
        ...
        ============================================================ --}}
        <section class="py-10 z-10 relative max-w-[100vw] bg-creme-leaks">
...
<div class="card-meta-container absolute top-0 right-0 bottom-0 w-1/3 flex flex-col p-6 text-right z-20 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                                ...
                                <div class="card-title-block absolute z-20 pointer-events-auto select-none transition-transform duration-500 group-hover:-translate-y-2">
                                    ...
                                    {{-- Wide: judul sangat besar, narrow: judul lebih kecil --}}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    window.syncFilmHeroVideos = syncHeroVideos;
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            navItems.forEach(item => {
                item.addEventListener('mouseenter', () => {
                    ...
                    if (window.syncFilmHeroVideos) {
                        window.syncFilmHeroVideos(Number(index));
                    }
