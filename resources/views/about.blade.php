...
@section('content')
    ...
    @if($introEnabled)
        {{-- ============================================================
        CINEMATIC INTRO OVERLAY (WEBP)
        ============================================================ --}}
        <div id="intro-overlay" class="fixed inset-0 z-[200] overflow-hidden" style="pointer-events: auto;">
            <img src="{{ asset('intro.webp') }}" alt="Intro Animation" class="w-full h-full object-cover" style="filter: none !important;">
        </div>
    @endif {{-- $introEnabled --}}
    ...
    <section id="about-hero" class="relative w-full overflow-hidden flex items-center"
        ...
        <div class="absolute inset-0 z-0">
            <img src="{{ isset($settings['about_hero_image']) ? asset($settings['about_hero_image']) : asset('photo/about_hero.png') }}"
                alt="Sinemaku Crew" class="w-full h-full object-cover">
            <div class="absolute inset-0 bg-black/40"></div>
        </div>

        <div class="relative z-20 w-full max-w-7xl mx-auto px-6 md:px-16 pt-20">
            <div class="max-w-2xl">
                <p id="hero-sub" class="font-sans text-white text-lg md:text-xl font-medium mt-8 mb-0 leading-none max-w-lg"
                ...
                --}}
                <h1 id="hero-tagline" class="font-sans font-bold text-white mt-2 mb-6"
                ...
                <div id="hero-actions" class="flex flex-row flex-wrap items-center gap-3 md:gap-5"
                    style="opacity: 0; transform: translateY(20px);">
                    <a href="#about-intro"
    ...
    <section id="about-teams-photo" class="relative w-full overflow-hidden" style="height: 100dvh; min-height: 400px;">
        <!-- Background Image -->
        <img src="{{ isset($settings['about_secondary_image']) ? asset($settings['about_secondary_image']) : asset('photo/about_secondary.png') }}"
            alt="Meet the team"
        ...
        <div class="max-w-[1540px] mx-auto">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 auto-rows-[250px]">
                ...
                <div class="gallery-item relative overflow-hidden group sm:col-span-2 sm:row-span-2 bg-gray-200">
                    <img src="{{ isset($settings['about_team_image_1']) ? asset($settings['about_team_image_1']) : asset('photo/about_hero.png') }}"
                        alt="Sinemaku Team"
                ...
                <div class="gallery-item relative overflow-hidden group bg-gray-200">
                    <img src="{{ isset($settings['about_team_image_2']) ? asset($settings['about_team_image_2']) : asset('photo/about_crew_1.png') }}"
                ...
                <div class="gallery-item relative overflow-hidden group row-span-2 bg-gray-200">
                    <img src="{{ isset($settings['about_team_image_3']) ? asset($settings['about_team_image_3']) : asset('photo/about_crew_2.png') }}"
                        alt="Set photo"
                        class="w-full h-full object-cover transition-transform duration-700 ease-out group-hover:scale-[1.03]">
                    <div
                        class="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                ...
                <!-- Item 4 -->
                <div class="gallery-item relative overflow-hidden group bg-gray-200">
                    <img src="{{ isset($settings['about_team_image_4']) ? asset($settings['about_team_image_4']) : asset('photo/about_crew_3.png') }}"
                        alt="Fun moment"
                        class="w-full h-full object-cover transition-transform duration-700 ease-out group-hover:scale-[1.03]">
                    <div
                        class="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                ...
                <div class="gallery-item relative overflow-hidden group sm:col-span-2 lg:col-span-2 bg-gray-200">
                    <img src="{{ isset($settings['about_team_image_5']) ? asset($settings['about_team_image_5']) : asset('photo/about_crew_4.png') }}"
                        alt="Sinemaku Event"
                        class="w-full h-full object-cover transition-transform duration-700 ease-out group-hover:scale-[1.03]"
                ...
                <!-- Item 6 -->
                <div class="gallery-item relative overflow-hidden group bg-gray-200">
                    <img src="{{ isset($settings['about_team_image_6']) ? asset($settings['about_team_image_6']) : asset('photo/about_crew_5.png') }}"
                        alt="Crew on set"
                        class="w-full h-full object-cover transition-transform duration-700 ease-out group-hover:scale-[1.03]">
                    <div
                        class="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
                ...
                <div class="bg-white p-10 border border-gray-100 hover:shadow-xl hover:shadow-black/5 transition-shadow duration-300 group"
                    ...
                    <h3 class="text-xl font-medium text-gray-900 mb-4">
                        {{ $settings['about_values_1_title'] ?? 'Film & Seri web' }}</h3>
                    <p class="text-gray-500 leading-relaxed font-light">
                        {{ $settings['about_values_1_body'] ?? 'Eksplorasi cerita layar lebar dan seri web dengan narasi segar, menghadirkan estetika visual yang menantang batas-batas konvensional.' }}
                    </p>
                </div>
                ...
                <div class="bg-white p-10 border border-gray-100 hover:shadow-xl hover:shadow-black/5 transition-shadow duration-300 group"
                    ...
                    <h3 class="text-xl font-medium text-gray-900 mb-4">
                        {{ $settings['about_values_2_title'] ?? 'Tayangan Televisi' }}</h3>
                    <p class="text-gray-500 leading-relaxed font-light">
                        {{ $settings['about_values_2_body'] ?? 'Menghadirkan kisah-kisah hangat untuk ruang keluarga melalui produksi televisi yang berkualitas dan dekat dengan realitas sehari-hari.' }}
                    </p>
                </div>
                ...
                <div class="bg-white p-10 border border-gray-100 hover:shadow-xl hover:shadow-black/5 transition-shadow duration-300 group"
                    ...
                    <h3 class="text-xl font-medium text-gray-900 mb-4">
                        {{ $settings['about_values_3_title'] ?? 'Komunitas & Event' }}</h3>
                    <p class="text-gray-500 leading-relaxed font-light">
                        {{ $settings['about_values_3_body'] ?? 'Rantai penghubung antarsineas dan penonton lewat Sinemaku Day, workshop, nobar bincang karya, dan program kerelawanan.' }}
                    </p>
                </div>
            </div>
        </div>
    ...
    @push('scripts')
        ...
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                ...
                /* ================================================================
                   WEBP ANIMATION INTRO LOGIC (Desktop Only)
                ================================================================ */
                if (INTRO_ENABLED && window.innerWidth > 768) {
                    ...
                    // Adjust this variable to match your animation's length (in milliseconds)
                    const ANIM_DURATION = 1800;

                    // Ensure hero variables are hidden beneath the overlay
                    if (overlay) {
                        gsap.set(heroTag, { opacity: 0 });
                        gsap.set('.hchar', { opacity: 0 });
                        gsap.set(heroSub, { opacity: 0, y: 20 });
                        gsap.set(heroAct, { opacity: 0, y: 20 });

                        // Create sequence timeline starting after animation duration
                        const tl = gsap.timeline({ delay: ANIM_DURATION / 1000 });
                        ...
                        // Cross-fade reveal hero section
                        tl.to(heroTag, { opacity: 1 }, "-=0.4");
                        tl.to('.hchar', {
                            opacity: 1,
                            duration: 0.7,
                            stagger: 0.018,
                        ...
                        // Fallback (if overlay is missing somehow)
                        gsap.set(heroTag, { opacity: 1 });
                        gsap.to('.hchar', { opacity: 1, duration: 0.7, stagger: 0.018, ease: 'power3.out', delay: 0.4 });
                        gsap.to(heroSub, { y: 0, opacity: 1, duration: 0.6, ease: 'power3.out', delay: 0.9 });
                        gsap.to(heroAct, { y: 0, opacity: 1, duration: 0.6, ease: 'power3.out', delay: 1.1 });
                    }
