...
<style>
  ...
  .oe-viewall {
    ...
    color: #111;
    text-decoration: none;
    font: 450 15px/1 Inter, system-ui;
    opacity: .92;
    ...
    <aside class="event-info reveal-x">
      ...
      <ul class="event-meta">
        ...
        <li>
          ...
          <span
            class="detail">{{ $event->harga ? 'Rp' . '' . str_replace(',', '.', number_format($event->harga)) : '' }}</span>
        </li>
      </ul>

      <a href="{{ $event->link }}" class="btn-primary" target="_blank" rel="noopener noreferrer">
        BUY NOW
...
<section class="other-events">
  <div class="oe-head reveal-x">
    ...
    <a class="oe-viewall" href="{{ route('event') }}">
      View All
      <svg viewBox="0 0 24 24" class="oe-arrow">
        <path d="M5 12h14m-6-7 7 7-7 7" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
          stroke-linejoin="round" />
      </svg>
    </a>
  </div>
    ...
    @foreach ($all_event as $item)
      <article class="oe-card">
        <a href="{{ route('detail-event', $item->slug) }}" class="oe-link">
          ...
          <time class="oe-date"
            datetime="{{ \Carbon\Carbon::parse($item->tgl_event)->format('Y-m-d') }}">{{ \Carbon\Carbon::parse($item->tgl_event)->format('M, d Y') }}</time>
        </a>
      </article>
    @endforeach
  </div>
