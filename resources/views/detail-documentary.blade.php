    ...
    <div id="editorial-wrapper" class="text-[#131b4d] relative w-full font-sans">
        ...
        <section class="detail-hero-section relative w-full h-[60vh] md:h-[75vh] flex flex-col justify-end overflow-hidden z-20">
            ...
            <div class="absolute inset-0 z-0">
                <video src="{{ asset('photo/' . $documentaries->photo) }}"
                    class="w-full h-full object-cover hero-parallax-img"
                    style="object-position: top center;"
                    autoplay muted loop playsinline preload="metadata"
                    aria-label="@i18n($documentaries, 'title')"></video>
...
<div class="detail-hero-content relative z-20 px-8 md:px-16 pb-12 max-w-[1400px] mx-auto w-full">
    <div class="flex flex-col items-start gap-2 mt-12 md:mt-16">
        @php
            $video_id = '';
            if (
                !empty($documentaries->link) &&
                preg_match('/(?:youtube\.com\/(?:[^\/]+\/.+\/|(?:v|e(?:mbed)?)\/|.*[?&]v=)|youtu\.be\/)([^"&?\/\s]{11})/i', $documentaries->link, $match)
            ) {
                $video_id = $match[1];
            }
        @endphp
