...
<style>
  ...
  .navbar-logo {
    ...
    color: #070707;
    text-shadow: 0 1px 5px rgba(0, 0, 0, .09);
  ...
  @media (max-width:640px) {
    :root {
      --shelf-col: clamp(220px, 78vw, 360px);
    }
  ...
  @media (max-width: 560px) {
    ...
    .shop-detail__title {
      ...
      margin-top: clamp(24px, 14vw, 68px);
    }
    ...
    :root {
      --shelf-col: clamp(220px, 82vw, 360px);
      --shelf-gap: 12px;
    }
    ...
    .shop-detail__cta {
      margin-top: clamp(14px, 4.8vw, 24px);
    }

    /* when multiple category containers stack, add spacing between blocks */
    .shop-detail__container+.shop-detail__container {
...
<section class="shop-detail">
  <div class="shop-detail__container">
    ...
    <div class="shop-detail__info">
      ...
      <div class="shop-detail__price">
        {{ $shop->harga ? 'Rp' . '' . str_replace(',', '.', number_format($shop->harga)) : '' }}</div>
          ...
          @if ($items->merchandise == $item->merchandise)
            <div class="product-card">
              <a href="{{ route('detail-shop', $items->slug) }}">
                <img src="{{ asset('photo/' . $items->photo) }}" alt="{{ $items->kategorishop }}">
                <p class="title">{{ $items->name }}</p>
                <p class="price">{{ $items->harga ? 'Rp' . '' . str_replace(',', '.', number_format($items->harga)) : '' }}</p>
              </a>
            </div>
          @endif
        @endforeach
