$(document).ready(function() {
    ...
    function handleOpeningSequence() {
        const sequence = $('#opening-sequence');
        ...
        timeouts.push(setTimeout(() => {
            if (isSkipped) return;
            $('#phase-logo').fadeOut(400, () => $('#phase-production').fadeIn(400));
        }, 2000));

        // Phase 3: Presenting
        timeouts.push(setTimeout(() => {
            if (isSkipped) return;
            $('#phase-production').fadeOut(400, () => $('#phase-presenting').fadeIn(400));
        }, 3800));
    ...
    $('#mega-menu-close').on('click', function() {
        $('#mega-menu').removeClass('visible opacity-100 pointer-events-auto')
    ...
    function handleNavbar() {
        ...
        const isHomepage = $('body').find('#hero-section').length > 0;
        let lastScrollY = window.scrollY;

        function toggleMenu(forceClose = false) {
            const isOpen = megaMenu.hasClass('visible');
            if (forceClose || isOpen) {
                ...
                $('#mega-menu .xl\\:col-span-8 > div > div').each(function(index) {
                    ...
                    $(this).css('transition-delay', `${delay}ms`).removeClass('opacity-0 -translate-x-8').addClass('opacity-100 translate-x-0');
                });
            }
             updateNavbarStyle();
        }
        ...
        function toggleSearch(forceClose = false) {
            const isOpen = searchModal.hasClass('visible');
             if (forceClose || isOpen) {
                $('body').css('overflow', 'unset');
        ...
        function updateNavbarStyle() {
            const isMenuOpen = megaMenu.hasClass('visible');
            const isSearchOpen = searchModal.hasClass('visible');
            const scrolled = window.scrollY > 50;

            const logo = $('#navbar-logo');
            const icons = $('#menu-toggle, #search-button');
        ...
        const searchData = { /* ... copy searchData object from Navbar.tsx ... */ };
        $('#search-input').on('keyup', function() {
            const query = $(this).val().toLowerCase();
            const resultsContainer = $('#search-results-container');
            // Implement search and render logic here...
            if (query.length > 2) {
                 resultsContainer.html(`<p class="text-white">Searching for "${query}"...</p>`);
            } else {
    ...
    function handleHomepage() {
        ...
        function showSlide(index) {
            // Content
            slides.filter(`[data-index=${currentSlide}]`).fadeOut(800, function() { $(this).addClass('hidden') });
            slides.filter(`[data-index=${index}]`).fadeIn(800).removeClass('hidden');
            
            // Background
            slideBgs.filter(`[data-index=${currentSlide}]`).removeClass('opacity-60').addClass('opacity-0');
            slideBgs.filter(`[data-index=${index}]`).removeClass('opacity-0').addClass('opacity-60');

            // Indicators
            indicators.filter(`[data-index=${currentSlide}]`).removeClass('bg-white scale-110').addClass('bg-white/30');
            indicators.filter(`[data-index=${index}]`).removeClass('bg-white/30').addClass('bg-white scale-110');
            
            currentSlide = index;
            
            // Counter
            const counterText = String(currentSlide + 1).padStart(2, '0') + ' — ' + String(slideCount).padStart(2, '0');
